...
def test_prompt_builder_contains_dimensions():
    ...
    config = AnalysisConfig(
        dimensions=(
            AnalysisDimension("business_model", "怎么赚钱（一句话）", "hint", 90),
            AnalysisDimension("moat", "护城河有多深（1-5分+一句话）", "", 90),
        )
    )
...
| --- | --- |
| 怎么赚钱（一句话） | 卖软件给企业 |
| 护城河有多深（1-5分+一句话） | 4分，客户切换成本高 |
"""
    dimensions = (
        AnalysisDimension("business_model", "怎么赚钱（一句话）", "", 90),
        AnalysisDimension("moat", "护城河有多深（1-5分+一句话）", "", 90),
    )
    parsed = parse_markdown_table(text, dimensions)
    assert parsed["business_model"] == "卖软件给企业"
